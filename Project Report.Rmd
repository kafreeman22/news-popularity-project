...
Information about the attributes used for analyses: 
...
c) How the data set was cleaned: 
...
- Pivoted all article catagory Boolean columns to single categorical column [Lifestyle, entertainment, business; social media, tech, world]
...
- Removed columns that are not relevant to our testing. The columns were: ['num_self_hrefs', 'kw_min_min', 'kw_max_min', kw_avg_min', 'kw_min_max', 'kw_max_max', 'kw_avE_max', 'kw_min_avg', 'kw_max_avg', kw_ave_avg', 'self_reference_min_shares','self _reference_max_shares', 'self_reference_avg_sharess' **is this correct?**, 'is_weekend", 'LDA_00', 'LDA_01', 'LDA_02', 'LDA_03', 'LDA_04', 'min_positive_polarity', 'max_positive_polarity', 'min_negative_polarity', ' max_negative_polarity, 'n_non_stop _words']
...
- Final Clean File used was 32971 rows × 24 columns




d) Software used: 
The data cleaning was done using Tableau Prep **add if there is anything else** and the statistical methods were analysed and graphed using R in RStudio. 





e) Questions to be analyzed:  
...
- Along with descriptive analytics, the use of anova showed visualizations with interesting patters. 
...
-Assumptions made and their results: 
  ...
  Variance - does not hold **why?what was done to change this? did it work?**
...
-We will not be adjusting for multiple testing since we are using different features for each of the listed questions **need to write??** 

**add anova graphs + explanations here**


f-ii) LINEAR REGRESSION:
...
-- Constant variance: **-------------------------?????**
-- Linearity: **-------------------------?????**
-- Normality: From the QQ plot, the normality assumption is not met. However, since there are large number of data points, the assumption of normality holds true. **check this answer again**
Response variable – number of article shares 
Independent variables – number of words in article, number of words in title, average token (word) length in article, article polarity, article subjectivity, rate of positive words, rate of negative words, day of publication (Monday-Sunday), article category (business, lifestyle, technology, world, entertainment, social media), number of images, number of videos, number of hyperlinks
...
g)Results. Describe your results clearly and concisely. Use graphical displays and 
...
h)Discussion. This section should briefly summarize the results and conclusions. Also 
...
i)References. List books or articles you consulted. (It is not necessary to do a lot of 
...
j)Tables and Figures. These can be included with the text in the body of the report 
...
k)Appendices. You may include more technical aspects of your analysis in an 
